        ...
        $(this.detalle.items).each(function(indice, fila){
            ...
            success: function (datas) {
                if(datas != 0){
                    ...
                    $('.stock_producto_'+indice).html('Stock: '+queda_stock);
                }else{
                    $('.stock_producto').html('');
                }
            }});
...
var anular_pedido = function(id_pedido){
    ...
    <form><input class="form-control text-center w-50" type="password" id="codigo_anular_venta" autocomplete="off"/></form><br>\
...
var impPreCuentaOpcional2 = function(id_pedido,id_mesa,estado,wiew){
    $.ajax({
        url: $("#url").val()+'venta/pedido_estado_update',
        type: "post",
        dataType: "json",
        ...
        if(wiew){
            printPdf($("#url").val()+'venta/impresion_precuenta_opcional_dos/'+id_pedido,true);
        }else{
            window.open($("#url").val()+'venta/impresion_precuenta_opcional_dos/'+id_pedido,'_blank');            
        }
    }); 
}
...
var descuento_factura = function(){
    ...
    if (editar_descuentos.length == 0) {
        ...
        <form class="dec">\
            ...
            <div class="display-descuento-motivo" style="display: block;">\
                <label class="m-t-10">Ingrese codigo de seguridad</label>\
                <input class="form-control text-center w-50" type="password" id="codigo_editar_descuentos" autocomplete="off"/>\
            </div>\
            </form>\
        </div>\
        <div><span class="text-success" style="font-size: 17px;">¿Está Usted de Acuerdo?</span></div>\
