    ...
    */
    ...
    public function buscar_cliente($data)
    {
        ...
        {        
            ...
            if($data['tipo_cliente'] == 1 AND $data['tipo_cliente'] == 2){
    ...
    */
    ...
    public function RegistrarVenta($data)
    {
        try
        {
            ...
            if ($data['billeterasMontos'][0] != 0) { $pago_lukita = $data['billeterasMontos'][0]; }
            if ($data['billeterasMontos'][1] != 0) { $pago_plin = $data['billeterasMontos'][1]; }
            if (isset($data['billeterasMontos'][2])) {
                if ($data['billeterasMontos'][2] != 0) { $pago_tunki = $data['billeterasMontos'][2]; }
            }
            if (isset($data['billeterasMontos'][3])) {
                if ($data['billeterasMontos'][3] != 0) { $pago_yape = $data['billeterasMontos'][3]; }                
            }
            ...
            if ($id_pago == 1) {
                if ($data['pago_efe'] > $data['total']) {
                    $pago_efe = $data['total'] + $data['comision_delivery'];
                }
            }
            ...
            if ($id_pago == 3) {
                ...
                if ($no_efectivo < $data['total']) {
                    $pago_efe = ($data['total']  + $data['comision_delivery']) - $no_efectivo;
                }
            }
        ...
        try 
        {
            ...
            foreach($data_producto as $item) {
                ...
                if ($cantidad == 1 OR $cantidad == $cantidad_eliminar) {
                    ...
                    $sql = "UPDATE tm_detalle_pedido SET cantidad = cantidad-" . $cantidad_eliminar . " WHERE id_pedido = " . $data['id_pedido'] . " AND id_pres = " . $data['id_pres'] . "";
                    $stm = $this->db->prepare($sql);
                    $stm->execute();                    

                    $sql = "UPDATE tm_detalle_pedido SET cant = cant-" . $cantidad_eliminar . " WHERE id_pedido = " . $data['id_pedido'] . " AND id_pres = " . $data['id_pres'] . "";
                    $stm = $this->db->prepare($sql);
                    $stm->execute();                    
