...
class Caja_Model extends Model
{
    ...
    public function monitor_ventas_list()
    {
        try
        {
            ...
            $stm = $this->db->prepare("SELECT *,IFNULL((pago_efe+pago_tar),0) AS monto_total, total AS monto_neto,(total + comis_del) AS total_completo FROM  v_ventas_con WHERE (fec_ven >= ? AND fec_ven <= ?) AND id_tped like ? AND id_tdoc like ? AND estado like ? ".$consulta."");
