    ...
    */
    public function stock_list($data)
    {
        try
        {
            ...
            $ffin = date('Y-m-d',strtotime($_POST['ffin']));
            ...
            foreach($c as $k => $d)
            {
                ...
                $c[$k]->{'Ingresos'} = $this->db->query("SELECT SUM(cant) AS ingresos FROM tm_inventario WHERE id_tipo_ope='3' and id_tipo_ins='".$d->id_tipo_ins."' and id_ins='".$d->id_ins."' and (date(fecha_r) >= '".$finicio."' AND date(fecha_r) <= '".$ffin."')")
                ->fetch(PDO::FETCH_OBJ);
                $c[$k]->{'Comprados'} = $this->db->query("SELECT SUM(cant) AS comprados FROM tm_inventario WHERE id_tipo_ope='1' and id_tipo_ins='".$d->id_tipo_ins."' and id_ins='".$d->id_ins."' and (date(fecha_r) >= '".$finicio."' AND date(fecha_r) <= '".$ffin."')")
                ->fetch(PDO::FETCH_OBJ);                
                $c[$k]->{'Salidas'} = $this->db->query("SELECT SUM(cant) AS salidas FROM tm_inventario WHERE id_tipo_ope='4' and id_tipo_ins='".$d->id_tipo_ins."' and id_ins='".$d->id_ins."' and (date(fecha_r) >= '".$finicio."' AND date(fecha_r) <= '".$ffin."')")
                ->fetch(PDO::FETCH_OBJ);    
                $c[$k]->{'Vendidos'} = $this->db->query("SELECT SUM(cant) AS vendidos FROM tm_inventario WHERE id_tipo_ope='2' and id_tipo_ins='".$d->id_tipo_ins."' and id_ins='".$d->id_ins."' and (date(fecha_r) >= '".$finicio."' AND date(fecha_r) <= '".$ffin."')")
                ->fetch(PDO::FETCH_OBJ);

                $c[$k]->{'Ingresos_SI'} = $this->db->query("SELECT SUM(cant) AS ingresos FROM tm_inventario WHERE id_tipo_ope='3' and id_tipo_ins='".$d->id_tipo_ins."' and id_ins='".$d->id_ins."' and (date(fecha_r) < '".$finicio."')")
                ->fetch(PDO::FETCH_OBJ);
                $c[$k]->{'Comprados_SI'} = $this->db->query("SELECT SUM(cant) AS comprados FROM tm_inventario WHERE id_tipo_ope='1' and id_tipo_ins='".$d->id_tipo_ins."' and id_ins='".$d->id_ins."' and (date(fecha_r) < '".$finicio."')")
                ->fetch(PDO::FETCH_OBJ);                 
                $c[$k]->{'Salidas_SI'} = $this->db->query("SELECT SUM(cant) AS salidas FROM tm_inventario WHERE id_tipo_ope='4' and id_tipo_ins='".$d->id_tipo_ins."' and id_ins='".$d->id_ins."' and (date(fecha_r) < '".$finicio."')")
                ->fetch(PDO::FETCH_OBJ);    
                $c[$k]->{'Vendidos_SI'} = $this->db->query("SELECT SUM(cant) AS vendidos FROM tm_inventario WHERE id_tipo_ope='2' and id_tipo_ins='".$d->id_tipo_ins."' and id_ins='".$d->id_ins."' and (date(fecha_r) < '".$finicio."')")
                ->fetch(PDO::FETCH_OBJ);
            }
            $data = array("data" => $c);
            $json = json_encode($data);
